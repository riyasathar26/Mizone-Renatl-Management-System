...
class BuildingInventory(models.Model):
    ...
    def _onchange_booking_type(self):
        ...
        elif self.booking_type == 'inventory':
            # Filter available products based on what hasn't been selected
            selected_product_ids = self.product_ids.ids
        ...
        for booking in self:
            ...
            if booking.booking_type == 'room' and booking.room_id:
                for product in booking.room_id.product_ids:
                    ...
                    line._onchange_product_ids()  # Trigger the onchange to compute price and subtotal
                    product_lines.append((0, 0, line._convert_to_write(line._cache)))
            elif booking.booking_type == 'inventory':
                # Filter available products based on what hasn't been selected
                selected_product_ids = booking.product_ids.ids
                available_products = self.env['product.product'].search([
                    ('id', 'not in', selected_product_ids)
                ])
    ...
    def action_print_booking(self):
        building_meterial = self.env.ref(
            'building.action_report_building_booking')
        print('building_meterial', building_meterial)
        if building_meterial:
            report_id = self.env.ref(
                'building.action_report_building_booking').id
            printer = self.env.ref(
                'building.action_report_building_booking').default_print_option
            rep_name = self.env.ref(
                'building.action_report_building_booking').report_name
            result = {'type': 'ir.actions.report', 'id': report_id,
                      'report_type': "qweb-pdf",
                      'binding_type': 'report',
                      'attachment': "",
                      'print_report_name': "(object._get_report_base_filename())",
                      'report_file': "report_building_booking",
                      'report_name': rep_name,
                      'default_print_option': printer,
                      'multi': 'false',
                      'binding_view_types': "list,form",
                      'model': 'building.booking',
                      'xml_id': "building.action_report_building_booking",
                      'context': ({'active_id': self.id,
                                   'active_ids': self.ids,
                                   'active_model': 'building.booking'})}
            return result
